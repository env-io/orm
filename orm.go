...
import (
	...
	"github.com/env-io/orm/clauses/order_clause"
	"github.com/env-io/orm/hints"
	"github.com/env-io/orm/utils"
)
...
func (o *ormBase) QueryM2MWithCtx(_ context.Context, md interface{}, name string) QueryM2Mer {
	DebugLog.Fatal("QueryM2MWithCtx is DEPRECATED. Use methods with `WithCtx` suffix on QueryM2M as replacement please.")
...
func (o *ormBase) QueryTableWithCtx(_ context.Context, ptrStructOrTableName interface{}) (qs QuerySeter) {
	DebugLog.Fatal("QueryTableWithCtx is DEPRECATED. Use methods with `WithCtx` suffix on QuerySeter as replacement please.")
...
func doTxTemplate(ctx context.Context, o TxBeginner, opts *sql.TxOptions,
	...
	defer func() {
		if panicked || err != nil {
			e := _txOrm.Rollback()
			if e != nil {
				DebugLog.Fatalf("rollback transaction failed: %v,%v", e, panicked)
			}
		} else {
			e := _txOrm.Commit()
			if e != nil {
				DebugLog.Fatalf("commit transaction failed: %v,%v", e, panicked)
			}
		}
	}()
	taskTxOrm := _txOrm
