...
var logger = log.New(os.Stdout, "[MIGRATION]", log.LstdFlags)
...
func Upgrade(lasttime int64) error {
	...
	for _, v := range sm {
		if _, ok := migs[v.name]; !ok {
			logger.Printf("start upgrade %s", v.name)
			v.m.Reset()
			v.m.Up()
			err := v.m.Exec(v.name, "up")
			if err != nil {
				logger.Printf("[Error] execute error: %s", err.Error())
				time.Sleep(2 * time.Second)
				return err
			}
			logger.Printf("end upgrade: %s", v.name)
			i++
		}
	}
	logger.Printf("total success upgrade: %d %s", i, "migration")
	time.Sleep(2 * time.Second)
	return nil
}
...
func Rollback(name string) error {
	if v, ok := migrationMap[name]; ok {
		...
		logger.Println("end rollback")
		time.Sleep(2 * time.Second)
		return nil
	}
	logger.Println("[Error] not exist the migrationMap name:" + name)
	time.Sleep(2 * time.Second)
...
func Reset() error {
	...
	for j := len(sm) - 1; j >= 0; j-- {
		...
		if isRollBack(v.name) {
			logger.Printf("skip the %s", v.name)
			time.Sleep(1 * time.Second)
			continue
		}
		logger.Printf("start reset: %s", v.name)
		v.m.Reset()
		v.m.Down()
		err := v.m.Exec(v.name, "down")
		if err != nil {
			logger.Printf("[Error] execute error: %s", err)
			time.Sleep(2 * time.Second)
			return err
		}
		i++
		logger.Printf("end reset: %s", v.name)
	}
	logger.Printf("total success reset: %d %s", i, "migration")
	time.Sleep(2 * time.Second)
	return nil
}
...
func isRollBack(name string) bool {
	...
	if err != nil {
		logger.Printf("[Error] get name has error %s", err.Error())
		return false
	}
	if num <= 0 {
		return false
